...
[[noreturn]] [[maybe_unused]] inline void unimplemented(const char* file,
                                                        int line,
                                                        const char* func) {
  std::cerr << "UNIMPLEMENTED: " << file << ":" << line << ":" << func << "()"
            << std::endl;
  std::abort();
}
...
void encode(const char* input_file_name, const char* output_file_name) {
  ...
  while (input_file.peek() != EOF) {
    ...
    while (bit_shift > 0) {
      bit_shift -= 8;
      output_file << static_cast<uint8_t>((in_number >> bit_shift) & 0xFF);
    }
  }
  input_file.close();
  output_file.close();
...
// converts an input file where a number is a sequence of 4 bytes into an
// output file of uint32_t
